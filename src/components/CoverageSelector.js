        ...
        return (
            <Form.Row>
                {gbbRes.result.lobData.personalAuto.offerings.filter(offer =>
                        ...
                        ? <Form.Group as={Col} controlId="formGridState"><Form.Label>{coverage.name}</Form.Label><Form.Control as="select">{coverage.terms[0].options.map(option=> <option>{option.name}</option>)}</Form.Control></Form.Group>
                        : null
                    )}
            </Form.Row>
        );
